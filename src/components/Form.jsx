  ...
  const login = () => {
    return tryLogin(email, password).then((res) => {
      if (res.error === "user not found") {
        ...
        const emailInLine = localStorage.setItem("email", email);
        const token = localStorage.setItem("token", res.token);
        {
            setCurrentUser({ token: token , email: emailInLine });
          }
  ...
  return (
    <form className="flex flex-col justify-evenly items-center border-2 border-green-500 w-80 h-96 rounded-xl p-4 text-white bg-green-500 bg-opacity-5 -sm:w-72 md:w-80 sm:w-72">
      ...
      <label htmlFor="messageError" className="w-full h-8 -mt-1">
        <p
          className="inline-block w-full text-white rounded-lg p-2 h-10 -mt-2  text-[13px] text-start "
          id="messageError"
        >
          {fail}
        </p>
      ...
      <p className="text-xs -mt-2">
        {text}
        <a href={goPath} className=" underline">
          {nameText}
        </a>
      </p>
      <Button
